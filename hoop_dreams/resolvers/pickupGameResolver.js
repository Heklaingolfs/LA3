...
// Leiðbeiningar fyrir PICKUP leiki

module.exports = {

    queries: {

        // Sækir alla pickup leiki í kerfinu
        allPickupGames: () =>
            new Promise((res, err) =>
                PickupGames.find({}, (error, games) => error ? err(new InternalServerError(false)) : res(games))) // Kastar villu InternalServer ef það nær ekki að tengjast
                .then(games => games)
                .catch(error => { throw error; }),

        // Sækir alla körfubolta velli eftir auðkenni
        pickupGame: (parent, args) =>
            new Promise((res, err) => {
                const { id } = args;
                PickupGames.findById(id, (error, game) => {
                    // Ath hvort auðkenni sé gild og sá leikur sé til með því auðkenni
                    if (error || game === null) {
    ...
    mutations: {
        ...
        // Býr til nýjan pickup leik í kerfið
        // Kastar villu ef einhverjar villur koma upp
         ...
         */
        // Bætir spilara við sem er til í pickup leik sem er til í kerfinu
        // Gerir það með því að skrá leikmann í hóp skráðra leikmanna í pickup leiknum og skráir pickup leikinn í leiki spilaða af leikmanninum
         ...
         */
        // Eyðir spilara sem er til í pickup leik sem er til í kerfinu
        // Gerir það með því að eyða leikmanninum í hóp skráðra leikmanna í pickup leiknum og eyðir pickup leiknum í leiki spilaða af leikmanninum
    ...
    types: {
        ...
        PickupGame: {
            ...
            // sækir staðsetningu hjá körufboltavöllum eftir auðkenni
            location: (parent) =>
